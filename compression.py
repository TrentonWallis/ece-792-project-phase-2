...
def generate_all_transformed_blocks(img, source_size, destination_size, step):
    ...
    for k in range((img.shape[0] - source_size) // step + 1):
        for l in range((img.shape[1] - source_size) // step + 1):
            ...
            transformed_blocks.extend([(k, l, direction, angle, apply_transformation(S, direction, angle)) for direction, angle in candidates])
    return transformed_blocks

def find_transformation(D,S):
    contrast, brightness = find_contrast_and_brightness2(D, S)
    S = contrast*S + brightness
    return [np.sum(np.square(D - S)), contrast, brightness]
...
def compress(img, source_size, destination_size, step):
    ...
    for i in range(i_count):
        ...
        for j in range(j_count):
            ...
            results = [find_transformation(D,S) for _, _, _, _, S in transformed_blocks]
            d = [r[0] for r in results] #errors
            best = d.index(min(d)) #index of best transformation
            tb = transformed_blocks[best]
            transformations[i][j] = (tb[0], tb[1], tb[2], tb[3], results[best][1], results[best][2])
...
def plot_img_difference(img, retrieved_img):
    plt.figure()
    plt.subplot(121)
...
def fractal_compress_rgb(img, reduction_factor, source_size, destination_size, step):
    img = reduce_rgb(img, reduction_factor)
    return compress_rgb(img, source_size, destination_size, step),img
...
def test_rgb():
    ...
    #reduce by 2 -> 32x32 then 
    transformations,img = fractal_compress_rgb(img,2,32,16,8)
    retrieved_img = decompress_rgb(transformations, 32, 16, 8)
    plot_img_difference(img, retrieved_img)
    return transformations
...
if __name__ == '__main__':
    ...
    # results = []
    # for i in range(5):
    #     results.append(pool.apply_async(test_rgb))
    
    # res = [r.get(timeout=100) for r in results]    
